          ...
          <>
            ...
            {session?.status === 'active' && (
              <>
                ...
                <Link to={`/agent/call/${id}`} className={btnClass('primary')}>Rejoin</Link>
              </>
            )}
          </>
        }
              ...
              {participants.map((p) => (
                <li key={p.id} className="flex items-center justify-between gap-3 py-3">
                  <div className="flex items-center gap-3">
                    <div className="grid h-9 w-9 place-items-center rounded-full bg-brand text-sm font-bold text-brand-fg">
